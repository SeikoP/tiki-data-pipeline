...
def check_category_path_in_db():
    ...
    try:
        ...
        connections = [
            {'host': 'localhost', 'database': 'crawl_data', 'user': 'postgres', 'password': 'postgres', 'port': 5432},
        ]
...
def check_extract_function():
    ...
    try:
        ...
        print(f"   • Category path: {result.get('category_path')} ({len(result.get('category_path', []))} levels)")
