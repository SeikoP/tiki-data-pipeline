...
from pipelines.crawl.crawl_products_detail import crawl_product_detail_with_selenium, extract_product_detail
import json
    ...
    try:
        ...
        if product:
            ...
            if name in category_path:
                print(f"   ❌ ERROR: Product name found in category_path!")
            else:
                print(f"   ✅ OK: Product name NOT in category_path")
                
            # Verify it looks like valid categories
            if len(category_path) > 0 and all(isinstance(c, str) and len(c) > 0 for c in category_path):
                print(f"   ✅ Category path looks valid ({len(category_path)} levels)")
            else:
                print(f"   ⚠️  Category path seems empty or invalid")
        else:
            print(f"   ⚠️  Failed to extract product data")
