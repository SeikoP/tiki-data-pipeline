...
def crawl_category_recursive(url, parent_url=None, level=0, max_level=2, max_categories_per_level=3, visited_urls=None, all_categories=None):
    """
    ...
    try:
        ...
        if level < max_level - 1:  # Không giới hạn ở level cuối
            ...
            if len(categories_with_images) < len([c for c in child_categories if c.get('image_url', '').strip()]):
                print(f"{'  ' * level}  ⚠ Giới hạn chỉ crawl {max_categories_per_level} danh mục đầu tiên (để test)")
        
        print(f"{'  ' * level}[Level {level}] ✅ Tìm thấy {len(categories_with_images)} danh mục con có hình ảnh")
...
def main():
    ...
    for category in unique_categories[:15]:  # Chỉ in 15 danh mục đầu
        ...
        parent_info = f" (Parent: {category.get('parent_url', 'N/A')})" if category.get('parent_url') else ""
