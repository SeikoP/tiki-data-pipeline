...
def test_crawl_products_sales_count():
    ...
    try:
        ...
        print(f"   - Có sales_count: {len(products_with_sales)} ({len(products_with_sales)/len(products)*100:.1f}%)")
        print(f"   - Không có sales_count: {len(products_without_sales)} ({len(products_without_sales)/len(products)*100:.1f}%)")
        ...
        if products_with_sales:
            ...
            for i, product in enumerate(products_with_sales[:5], 1):
                ...
                sales_str = f"{sales_count:,}" if isinstance(sales_count, (int, float)) else str(sales_count)
...
def test_crawl_product_detail_sales_count():
        ...
        if os.path.exists(test_file):
            with open(test_file, 'r', encoding='utf-8') as f:
                ...
                if products:
                    ...
                    products_with_sales = [p for p in products if p.get('url') and p.get('sales_count') is not None]
                    products_without_sales = [p for p in products if p.get('url') and p.get('sales_count') is None]
                    ...
                    test_urls.extend([p.get('url') for p in products_without_sales[:2] if p.get('url')])
    ...
    for i, url in enumerate(test_urls, 1):
        ...
        try:
            ...
            if not html_content or len(html_content) < 100:
                print(f"   ❌ HTML content quá ngắn: {len(html_content) if html_content else 0} ký tự")
                continue
            ...
            print(f"      - Tên: {name[:60]}...")
            sales_str = f"{sales_count:,}" if isinstance(sales_count, (int, float)) else (str(sales_count) if sales_count is not None else 'N/A')
            print(f"      - Sales count: {sales_str}")
            ...
            if detail.get('rating', {}).get('average'):
                print(f"      - Đánh giá: {detail.get('rating', {}).get('average')}/5 ({detail.get('rating', {}).get('total_reviews')} reviews)")
    ...
    print(f"   - Có sales_count: {with_sales} ({with_sales/total*100:.1f}%)" if total > 0 else "   - Có sales_count: 0")
    print(f"   - Không có sales_count: {without_sales} ({without_sales/total*100:.1f}%)" if total > 0 else "   - Không có sales_count: 0")
...
def main():
    ...
    if products:
        ...
        print(f"   - Có sales_count: {products_with_sales} ({products_with_sales/len(products)*100:.1f}%)")
    
    if detail_results:
        detail_with_sales = len([r for r in detail_results if r.get('has_sales_count')])
        print(f"\n✅ Test 2 (Crawl Product Detail):")
        print(f"   - Tổng: {len(detail_results)} products")
        print(f"   - Có sales_count: {detail_with_sales} ({detail_with_sales/len(detail_results)*100:.1f}%)")
