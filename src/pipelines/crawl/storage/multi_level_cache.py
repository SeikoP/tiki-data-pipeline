...
class MultiLevelCache:
    ...
    def cache_product_detail(self, product_id: str, detail: dict, ttl: Optional[int] = None) -> bool:
        ...
        return self.set(
            key,
            {"product_id": product_id, "detail": detail, "cached_at": time.time()},
            ttl=ttl
        )
    ...
    def get_stats(self) -> dict:
        ...
        if total_requests > 0:
            stats['overall_hit_rate'] = f"{(total_requests - stats['misses']) / total_requests * 100:.2f}%"
        else:
...
# Singleton instance
_multi_level_cache_instance: Optional[MultiLevelCache] = None
