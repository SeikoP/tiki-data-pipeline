...
spec = importlib.util.spec_from_file_location("extract_category_link_selenium", 
...
def crawl_single_category(url, parent_url, level, max_level, visited_urls, cache_dir='data/raw/cache'):
    """
        ...
        if cache_file:
            try:
                with open(cache_file, 'w', encoding='utf-8') as f:
                    json.dump({'url': url, 'categories': categories_with_images}, f, ensure_ascii=False, indent=2)
            except:
                pass
    ...
    with tqdm(total=len(tasks), desc=desc, unit="danh mục") as pbar:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for url, parent_url in tasks:
                future = executor.submit(crawl_single_category, url, parent_url, level, max_level, visited_urls)
            ...
            for future in as_completed(future_to_url):
                ...
                try:
                    ...
                    if success:
                        ...
                        with stats_lock:
                            pbar.set_postfix({'✅': stats['total_success'], '❌': stats['total_failed']})
                        print(f"\n  ❌ Lỗi crawl {url}: {error}")
                except Exception as e:
                    error_msg = str(e)
...
def crawl_category_recursive_optimized(root_url, max_level=3, max_workers=3, visited_urls=None, all_categories=None):
    """
    ...
    for current_level in range(max_level + 1):
        ...
        results = crawl_level_parallel(
            new_urls, new_parents, current_level, max_level, 
            visited_urls, max_workers=max_workers
        )
