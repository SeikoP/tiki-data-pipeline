...
class StarSchemaBuilderV2:
    ...
    def __init__(self, db_host: str = "localhost", db_port: int = 5432,
                 ...
                 target_db: str = "tiki_warehouse", db_user: str = "postgres",
    ...
    def connect(self):
        ...
        try:
            self.source_conn = psycopg2.connect(
                host=self.db_host, port=self.db_port, database=self.source_db,
            ...
            try:
                self.target_conn = psycopg2.connect(
                    host=self.db_host, port=self.db_port, database=self.target_db,
                    user=self.db_user, password=self.db_password
                )
                ...
                temp_conn = psycopg2.connect(
                    host=self.db_host, port=self.db_port, database='postgres',
                    user=self.db_user, password=self.db_password
                )
    ...
    def create_schema(self):
        ...
        tables = ["fact_product_sales", "dim_price_segment", "dim_date", "dim_brand", 
        ...
        # Create indexes
        self.target_cur.execute("CREATE INDEX idx_fact_product_sk ON fact_product_sales(product_sk)")
        self.target_cur.execute("CREATE INDEX idx_fact_category_sk ON fact_product_sales(category_sk)")
    ...
    def load_data(self, products: List[dict]):
        ...
        logger.info(f"  Filtering: {len(products)} → {len(filtered_products)} products (removed {len(products) - len(filtered_products)} with NULL brand/seller)")
        ...
        for idx, prod in enumerate(filtered_products, 1):
            try:
                ...
                if pid not in prod_cache:
                    ...
                    """, (pid, prod['name'][:500], (prod.get('brand') or 'Unknown')[:255], 
                          prod.get('url', '')[:500]))
                ...
                if cat_key not in cat_cache:
                    ...
                    """, (cat_id, levels[0], levels[1], levels[2], levels[3], levels[4], 
                ...
                """, (prod_sk, cat_sk, seller_sk, brand_sk, date_sk, price_seg_sk,
                      price, orig_price, discount, prod.get('sales_count', 0),
                      revenue, profit, prod.get('rating', 0), 0, prod.get('review_count', 0)))
    ...
    def verify(self):
        ...
        for table in ['dim_product', 'dim_category', 'dim_seller', 'dim_brand', 'dim_date', 'dim_price_segment', 'fact_product_sales']:
