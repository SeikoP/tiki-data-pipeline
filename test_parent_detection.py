...
if len(result1['category_path']) == 4 and result1['category_path'][0] == 'Nhà Cửa - Đời Sống':
    ...
    print(f"⚠️  WARN: 4 levels but parent is '{result1['category_path'][0]}' (expected 'Nhà Cửa - Đời Sống')")
...
html2 = create_test_html(
    ["Decal & giấy dán kính", "Tranh dán kính", "Tranh phong cảnh"],
    "Tranh Dán Tường"
)
...
if len(result2['category_path']) == 4 and result2['category_path'][0] == 'Nhà Cửa - Đời Sống':
    ...
    print(f"⚠️  WARN: 4 levels but parent is '{result2['category_path'][0]}' (expected 'Nhà Cửa - Đời Sống')")
...
html3 = create_test_html(
    ["Trang trí nhà cửa", "Tranh trang trí", "Tranh đá", "Tranh thiên nhiên"],
    "Tranh Núi"
)
...
if len(result3['category_path']) == 5 and result3['category_path'][0] == 'Nhà Cửa - Đời Sống':
    ...
    print(f"⚠️  WARN: 5 levels but parent is '{result3['category_path'][0]}' (expected 'Nhà Cửa - Đời Sống')")
...
print("SUMMARY")
print("=" * 80)
print(f"Test 1 (3 levels): {len(result1['category_path'])} levels - {'✅' if len(result1['category_path']) == 4 else '❌'}")
print(f"Test 2 (3 levels different): {len(result2['category_path'])} levels - {'✅' if len(result2['category_path']) == 4 else '❌'}")
print(f"Test 3 (4 levels): {len(result3['category_path'])} levels - {'✅' if len(result3['category_path']) == 5 else '⚠️'}")
