...
class CIRunner:
    ...
    def __init__(self, verbose: bool = False, parallel: bool = True, config_file: Optional[str] = None):
    ...
    def install_dependencies(self) -> None:
        ...
        self._run_command(
            [self.python_cmd, "-m", "pip", "install"] + dev_packages,
    ...
    def lint(self, fix: bool = False) -> TaskResult:
        ...
        try:
            ...
            cmd.extend(["src/", "tests/", "airflow/dags/"])
    ...
    def format_code(
        ...
        use_ruff = use_ruff if use_ruff is not None else (fmt_config.get("formatter", "ruff") == "ruff")
        ...
        skip_string_normalization = skip_string_normalization if skip_string_normalization is not None else fmt_config.get("skip_string_normalization", False)
        
        try:
            self._print_info("✨ Formatting code with modern formatters...")
            
            paths = ["src/", "tests/", "airflow/dags/", "ci.py"]
            
            # Step 1: Import sorting with isort
            self._print_info("📦 Sorting imports with isort...")
            isort_cmd = [
                self.python_cmd, "-m", "isort",
                "--profile", "black",  # Compatible with Black/Ruff
    ...
    def type_check(self) -> TaskResult:
        ...
        try:
            ...
            self._run_command(
                [self.python_cmd, "-m", "mypy", "src/", "--ignore-missing-imports"],
                "Type check"
            )
    ...
    def security_check(self) -> TaskResult:
        ...
        try:
            ...
            try:
                self._run_command(
                    [self.python_cmd, "-m", "safety", "check"],
                    "Safety check",
                    check=False
                )
            except Exception:
    ...
    def perf_check(self) -> TaskResult:
        ...
        try:
            ...
            self._run_command(
                [self.python_cmd, "-m", "ruff", "check", "--select", "PERF",
                 "src/", "tests/", "airflow/dags/"],
                "Performance check",
                check=False
            )
    ...
    def dead_code_check(self) -> TaskResult:
        ...
        try:
            ...
            self._run_command(
                [self.python_cmd, "-m", "vulture", "src/", "airflow/dags/",
                 "--min-confidence", "80"],
                "Dead code check",
                check=False
            )
    ...
    def complexity_check(self) -> TaskResult:
        ...
        try:
            ...
            self._run_command(
                [self.python_cmd, "-m", "radon", "cc", "src/", "airflow/dags/", "--min", "B"],
                "Cyclomatic complexity",
                check=False
            )
            ...
            self._run_command(
                [self.python_cmd, "-m", "radon", "mi", "src/", "airflow/dags/", "--min", "B"],
                "Maintainability index",
                check=False
            )
...
"""
    ...
    def _display_results(self) -> None:
        ...
        if RICH_AVAILABLE and self.console:
            ...
            print("=" * 70)
            for result in self.results:
                print(f"{result.name:<20} {result.status.value:<10} "
...
def main():
    ...
    parser.add_argument(
        "--skip-string-norm",
        action="store_true",
        help="Skip string quote normalization"
    )
