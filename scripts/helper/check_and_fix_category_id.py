...
def check_and_fix_category_id():
    ...
    )  # DEVELOPMENT ONLY  # trufflehog:ignore
    ...
    try:
        ...
        if products_with_category_url > products_with_category_id_after:
            ...
            print(
                f"   ⚠️  Còn {missing} products có category_url nhưng không extract được category_id"
            )
            print("      (có thể do format URL không đúng pattern /c[0-9]+)")
