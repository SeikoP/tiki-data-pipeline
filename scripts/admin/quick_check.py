...
def quick_check():
    ...
    try:
        ...
        if not isinstance(result, dict):
            ...
            print(f"   ✅ Extract function OK (got {len(result.get('category_path', []))} category levels)")
    except Exception as e:
    ...
    try:
        import psycopg2
        try:
            conn = psycopg2.connect(host='localhost', database='crawl_data', user='postgres', password='postgres', port=5432)
            cur = conn.cursor()
