...
def verify_optimizations():
    ...
    if redis_file.exists():
        content = redis_file.read_text(encoding="utf-8")
        checks["Redis connection pooling"] = (
            "connection_pool" in content and "max_connections" in content
        )
    ...
    if all_passed:
        ...
        print(
            "      docker exec postgres psql -U postgres -d crawl_data -f /docker-entrypoint-initdb.d/add_performance_indexes.sql"
        )
        print()
