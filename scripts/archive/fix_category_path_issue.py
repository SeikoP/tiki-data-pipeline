...
def run_script(script_name: str, description: str) -> bool:
    ...
    try:
        result = subprocess.run(
            [sys.executable, str(script_path)], check=True, capture_output=False, text=True
        )
...
def main():
    ...
    if not run_script(
        "enrich_categories_with_paths.py", "Enrich categories file với category_id và category_path"
    ):
        print(f"\n{RED}❌ Bước 1 thất bại!{END}")
        return False

    # Step 2: Apply schema changes
    if not run_script("apply_schema_changes.py", "Apply database schema changes"):
    ...
    print("   1. ✅ Enriched categories file với category_id và category_path")
    print("   2. ⚠️ Applied database schema changes (hoặc sẽ apply khi DB ready)")
