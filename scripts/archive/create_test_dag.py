...
content = content.replace(
    '"max_active_tasks": 10,', '"max_active_tasks": 3,  # Giảm xuống 3 cho test'
)
...
pattern = (
    r'(max_pages = int\(\s+Variable\.get\("TIKI_MAX_PAGES_PER_CATEGORY", default_var="20"\)\s+\))'
)
...
# 10. Giảm retries
content = content.replace('"retries": 3,', '"retries": 1,  # TEST MODE: Giảm retries')
