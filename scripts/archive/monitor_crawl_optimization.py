...
def get_product_count():
    ...
    try:
        ...
        result = subprocess.run(
            cmd, shell=True, capture_output=True, text=True, cwd="e:\\Project\\tiki-data-pipeline"
        )
...
def monitor_crawl(duration_minutes=60):
    ...
    for minute in range(duration_minutes):
        ...
        print(
            f"[{minute+1:2d}/{duration_minutes}] {datetime.now().strftime('%H:%M:%S')} | ", end=""
        )
    ...
    print("Expected Baseline: 300-500 products/hour")
