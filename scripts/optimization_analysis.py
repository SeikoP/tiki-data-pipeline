...
def check_file_size_and_structure():
    ...
    if categories_file.exists():
        ...
        checks.append({
            ...
            "items": [
                (GREEN if has_category_id else RED, f"category_id: {'✅' if has_category_id else '❌'}"),
                (GREEN if has_category_path else RED, f"category_path: {'✅' if has_category_path else '❌'}"),
            ]
        })
    ...
    if products_file.exists():
        ...
        checks.append({
            ...
            "items": [
                ...
                (GREEN if with_path == len(products) else YELLOW, f"Products with category_path: {with_path}/{len(products)}"),
            ]
        })
...
def check_data_pipeline_optimization():
    ...
    if loader_file.exists():
        ...
        print(f"📊 Upsert Strategy: {GREEN if has_upsert else RED}{'✓ ON CONFLICT' if has_upsert else '✗'}{END}")
        checks.append({
    ...
    if transformer_file.exists():
        ...
        print(f"📊 Data Validation: {GREEN if has_validation else RED}{'✓' if has_validation else '✗'}{END}")
        checks.append({
...
def check_code_quality():
    ...
    python_files = list(Path("src/pipelines").glob("**/*.py")) + list(Path("airflow/dags").glob("*.py"))
